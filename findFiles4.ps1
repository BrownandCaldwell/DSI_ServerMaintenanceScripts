...
$path = "C:\Users\Administrator\OneDrive - Brown and Caldwell"
        ...
        ForEach ($item in $Path) {
            Try {
                ...
                Invoke-Expression $Script | ForEach {
                    Try {
                        If ($_.Replace("[","\[").Replace("]","\]").Trim() -match "^(?<Size>\d+)\s(?<Date>\S+\s\S+)\s+(?<FullName>.*)") {
...
$fname = $scriptDir + "\Files.csv"
$zipFName = $scriptDir + "\" + $env:COMPUTERNAME + "_CompressedFiles.zip"
...
Get-FolderItem $path | Export-Csv -Path $fname -NoTypeInformation

Add-Type -Assembly 'System.IO.Compression.FileSystem'
$zip = [System.IO.Compression.ZipFile]::Open($zipFName, "Create")
[System.IO.Compression.ZipFileExtensions]::CreateEntryFromFile($zip, $fname, $fname)
$zip.Dispose()

robocopy .\ \\keydbsdev.bc.com\Branch2Cloud $zipFName /r:0 
